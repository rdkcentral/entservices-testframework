...
extern "C" CURLcode __wrap_curl_easy_setopt(CURL* curl, CURLoption option, void* param)
{
    return Wraps::getInstance().curl_easy_setopt(curl, option, param);
}

extern "C" CURLcode __wrap_curl_easy_perform(CURL* curl)
{
    return Wraps::getInstance().curl_easy_perform(curl);
}

extern "C" CURLcode __wrap_curl_easy_getinfo(CURL* curl, CURLINFO info, long* value)
{
    return Wraps::getInstance().curl_easy_getinfo(curl, info, value);
}

extern "C" const char* __wrap_curl_easy_strerror(CURLcode errornum)
{
    return Wraps::getInstance().curl_easy_strerror(errornum);
}

extern "C" CURL* __wrap_curl_easy_init(void)
{
    return Wraps::getInstance().curl_easy_init();
}

extern "C" void __wrap_curl_easy_cleanup(CURL* handle)
{
    return Wraps::getInstance().curl_easy_cleanup(handle);
}

extern "C" FILE *__wrap_fopen(const char *pathname, const char *mode)
{
...
{
    EXPECT_NE(impl, nullptr);
    return impl->curl_easy_setopt(curl, option, param);
}

CURLcode Wraps::curl_easy_perform(CURL* curl)
{
    EXPECT_NE(impl, nullptr);
    return impl->curl_easy_perform(curl);
}

CURLcode Wraps::curl_easy_getinfo(CURL* curl, CURLINFO info, long* value)
{
    EXPECT_NE(impl, nullptr);
    return impl->curl_easy_getinfo(curl, info, value);
}

const char* Wraps::curl_easy_strerror(CURLcode errornum)
{
    EXPECT_NE(impl, nullptr);
    return impl->curl_easy_strerror(errornum);
}

CURL* Wraps::curl_easy_init(void)
{
    EXPECT_NE(impl, nullptr);
    return impl->curl_easy_init();
}

void Wraps::curl_easy_cleanup(CURL* handle)
{
    EXPECT_NE(impl, nullptr);
    return impl->curl_easy_cleanup(handle);
}
